...
class HACLEvaluator {
	...
	public static function whitelist( $title, $user, &$whitelisted ) {
		...
		if (HACLWhitelist::isInWhitelist($articleID)) {
			...
			self::finishLog("Read access was determined by the Whitelist.", true, true);
...
//$result = true;
	...
	public static function onDiffViewHeader(DifferenceEngine $diffEngine, $oldRev, $newRev) {
		...
		$newText = $diffEngine->mNewTags;
		if (empty($newText)) {

			$diffEngine->loadText();
		}
		$newText = $diffEngine->mNewTags;
		$oldText = $diffEngine->mOldTags;

	
		global $wgParser;
		$options = new ParserOptions;

		$output = $wgParser->parse($newText, $diffEngine->mNewPage, $options);
